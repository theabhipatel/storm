...
// At-least-once: select unpublished rows, publish, then mark publishedAt.
// If we crash between publish and mark, the next tick re-publishes — consumers
// dedup on eventId (the outbox row id, which is in the envelope).
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          // Stop the batch on first failure so we back off and retry on the next tick.
          break;
        }
      }
      if (published > 0) {
